from PIL import Image, UnidentifiedImageError
from bs4 import BeautifulSoup
import os
import requests

def descargar_imagen(codigo, alumno):
    #URL de la imagen
    url = 'http://websecgen.unmsm.edu.pe/carne/imagenes-UNMSM/20/2/00100'+codigo+'.jpg'

    #Nombre de la imagen
    name = codigo + '.jpg'

    #Creando carpeta si no existe
    folder = "ATTENDANCE/image_folder/"
    if not os.path.exists(folder):
        os.makedirs(folder)

    #Descargar la imagen y guardarlo en la variable img
    try:
        response = requests.get(url, stream=True, timeout=10)
        response.raise_for_status()

        img = Image.open(response.raw)

        #Guardo la imagen en la carpeta respectiva
        img.save(folder + name)
        print(f'Imagen guardada como {name}')
    except (requests.HTTPError, requests.RequestException) as e:
        print(f'Error al descargar la imagen: {e}')
    except UnidentifiedImageError:
        print('El contenido descargado no es una imagen válida.')
    except Exception as e:
        print(f'Ocurrió un error inesperado: {e}')

CARNETS_ENDPOINT = 'http://websecgen.unmsm.edu.pe/carne/carne.aspx'

CARNET_REQUEST_HEADERS = {
    'accept': '*/*',
    'accept-language': 'en-US,en;q=0.9,es;q=0.8',
    'cache-control': 'no-cache',
    'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
    'cookie': 'AMCV_4D6368F454EC41940A4C98A6%40AdobeOrg=-2121179033%7CMCIDTS%7C19243%7CMCMID%7C58505380327749689480958356562107143857%7CMCAAMLH-1663130574%7C4%7CMCAAMB-1663130574%7CRKhpRz8krg2tLO6pguXWp5olkAcUniQYPHaMWWgdJ3xzPWQmdj0y%7CMCOPTOUT-1662532974s%7CNONE%7CMCAID%7CNONE%7CMCSYNCSOP%7C411-19250%7CvVersion%7C5.3.0; s_pers=%20v8%3D1662525836184%7C1757133836184%3B%20v8_s%3DFirst%2520Visit%7C1662527636184%3B%20c19%3Dpr%253Apure%2520portal%253Apersons%253Aview%7C1662527636192%3B%20v68%3D1662525835235%7C1662527636221%3B; __utma=165303783.506084233.1662658626.1662959785.1662959785.1; __utmz=165303783.1662959785.1.1.utmcsr=google|utmccn=(organic)|utmcmd=organic|utmctr=(not%20provided); amp_adc4c4=QdLyo8-lTWUepkJP1Mhspq...1gdgcirqu.1gdgcj8ie.0.0.0; amp_d915a9=c8XdfPgsTQ5w40mSx7hk-Z.MVRoVFVINGtuUmFVV00wSmJCbm9XZVJUb2lGMg==..1gdgcis3s.1gdgcj8od.0.12.12; ASP.NET_SessionId=pmnqtyru0l4pq0il34ikslby; __utma=85720863.506084233.1662658626.1664578414.1664578414.1; __utmc=85720863; __utmz=85720863.1664578414.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none); tk_or=%22https%3A%2F%2Fwww.google.com%2F%22; tk_lr=%22https%3A%2F%2Fwww.google.com%2F%22; _ga_D366LERZQD=GS1.1.1668177583.1.1.1668181001.0.0.0; _ga=GA1.1.506084233.1662658626; _ga_LEJ0F8DTRG=GS1.1.1668271478.4.1.1668272444.0.0.0; _ga_8GFJVSE3W0=GS1.1.1668414439.20.0.1668415684.0.0.0',
    'Referer': CARNETS_ENDPOINT,
    'Referrer-Policy': 'strict-origin-when-cross-origin',
    'connection': 'keep-alive',
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:101.0) Gecko/20100101 Firefox/101.0',
}

def carnet_request_body(student_code):
    return f"""ctl00%24ScriptManager1=ctl00%24ContentPlaceHolder1%24UpdatePanel1%7Cctl00%24ContentPlaceHolder1%24cmdConsultar&__EVENTTARGET=&__EVENTARGUMENT=&ctl00%24ContentPlaceHolder1%24txtUsuario={student_code}&ctl00%24ContentPlaceHolder1%24txtFacultad=FACULTAD%20DE%20INGENIER%C3%8DA%20DE%20SISTEMAS%20E%20INFORM%C3%81TICA&ctl00%24ContentPlaceHolder1%24txtPrograma=Ingenier%C3%ADa%20de%20Software&ctl00%24ContentPlaceHolder1%24txtAlumno=RODRIGO%20JOSE%20ALVA%20S%C3%81ENZ&ctl00%24ContentPlaceHolder1%24NoBot1%24NoBot1_NoBotExtender_ClientState=CHALLENGE&ctl00%24ContentPlaceHolder1%24TextBoxWatermarkExtender1_ClientState=&ctl00%24ContentPlaceHolder1%24MaskedEditExtender1_ClientState=&__VIEWSTATE=%%%2FGkDjK8QBwC0TkDfk%3D&__VIEWSTATEGENERATOR=ABB7F4A4&__EVENTVALIDATION=%2FwEWCQLkuJrXDgKnqqeUBAKikZQ3AvmukKoFAu2%2BtNsDAu7ClKsEArOCz7YHAsmmlswJAtHui4oEQBJjTMXt6HxY6rawAtqEbCIrLv0%3D&__ASYNCPOST=true&ctl00%24ContentPlaceHolder1%24cmdConsultar=Consultar"""

STUDENT_HTML_CLASSNAME = 'ctl00_ContentPlaceHolder1_txtAlumno'
FACULTY_HTML_CLASSNAME = 'ctl00_ContentPlaceHolder1_txtFacultad'

def carnet_name_request(student_code):
    response = requests.post(
        CARNETS_ENDPOINT,
        headers=CARNET_REQUEST_HEADERS,
        data=carnet_request_body(student_code),
        timeout=10
    )

    soup = BeautifulSoup(response.text, 'html.parser')
    
    name_input = soup.find('input', {'id': STUDENT_HTML_CLASSNAME})
    faculty_input = soup.find('input', {'id': FACULTY_HTML_CLASSNAME})

    if not name_input or not faculty_input:
        raise ValueError('Could not find student name')

    fullname = name_input.get('value', '').strip()
    faculty = faculty_input.get('value', '').strip()

    if not fullname or not faculty:
        return None
    return {'fullname': fullname, 'faculty': faculty}

relacionAlumnos = {}

for i in range(22200067,22200108):
    cod = str(i)
    try:
        res = carnet_name_request(cod)
        fullname = res.get('fullname')
        relacionAlumnos[cod] = fullname
        descargar_imagen(cod, fullname)
    except Exception as e:
        print(f'Error con el código')
    
for i in range(22200228,22200310):
    cod = str(i)
    try:
        res = carnet_name_request(cod)
        fullname = res.get('fullname')
        relacionAlumnos[cod] = fullname
        descargar_imagen(cod, fullname)
    except Exception as e:
        print(f'Error con el código')

print(relacionAlumnos)